...
- HTTP работает по схеме _запрос → ответ_.
...
<details>
<summary><b>Что такое Short Polling?</b></summary>

`Short Polling` - простое периодическое опрашивание веб-сервера посредством отправки классических HTTP запросов.

### Преимущества:

Самый простой в реализации подход, может быть реализован практически на любой платформе с любым языком программирования.

## Недостатки:

Самый неэффективный подход во всех отношениях, однако не требующий никаких дополнительных действий для реализации. Создает ненужные запросы в случае отсутствия обновлений данных. Высокая нагрузка на сервер, таймауты при передаче данных.
...
<details>
<summary><b>Что такое Long Poling?</b></summary>

`Long Polling` был одним из первых методов для обновления контента на странице в реальном времени до появления более современных технологий, таких как `WebSocket` и `Server-Sent Events`. Это способ обхода ограничений традиционного веб-протокола HTTP, который не поддерживает двустороннюю связь.

### Принцип работы следующий:

Клиент отправляет на сервер HTTP запрос, сервер выполняет запрос и может отправлять несколько порций данных перед отправкой финального результата и закрытием соединения.

### Преимущества:

Простота в понимании и реализации, асинхронная передача данных со стороны сервера.
...
- Дока ([https://learn.javascript.ru/long-polling](https://learn.javascript.ru/long-polling))
- Статья ([https://dev.to/debajit13/short-polling-vs-long-polling-a-comprehensive-guide-for-developers-31j3](https://dev.to/debajit13/short-polling-vs-long-polling-a-comprehensive-guide-for-developers-31j3))
</details>

---

<details>
<summary><b>Что такое gRPC?</b></summary>
`gRPC` (Google Remote Procedure Call) — это высокопроизводительный фреймворк с открытым исходным кодом от Google для быстрого обмена данными между приложениями и микросервисами.

Ключевая особенность gRPC заключается в том, что клиент может вызывать функцию на удаленном сервере так же легко, как если бы это была локальная программа, что делает архитектуру распределенных систем более чистой.

### Основные составляющие технологии:

- Protocol Buffers (Protobuf): Язык описания интерфейсов и формат сериализации данных. Заменяет привычные JSON и XML. Данные сжимаются в компактный бинарный вид, благодаря чему парсинг происходит значительно быстрее, а трафик экономится.
- HTTP/2 как транспорт: Использует преимущества протокола HTTP/2 — сжатие заголовков, мультиплексирование (множество запросов по одному соединению без задержек) и потоковую передачу (streaming).
- Генерация кода: По написанному текстовому .proto файлу (контракту) специальный компилятор автоматически создает готовый код клиента и сервера на множестве языков программирования (Python, Go, Java, C#, C++ и других).

### Главные отличия gRPC от REST API:

- Способ вызова: REST работает с "ресурсами" через методы (GET, POST), а gRPC ориентирован на "вызовы процедур" (например, функция GetUser).
- Скорость и трафик: gRPC работает значительно быстрее за счет бинарного формата Protobuf и сетевых возможностей HTTP/2.
- Строгая типизация: Контракт строго задает форматы данных, предотвращая ошибки на этапе компиляции.

Чаще всего gRPC применяется в микросервисной архитектуре, где требуется высокая скорость обработки межсервисных запросов, в потоковой передаче данных (стриминге) и для связи мобильных клиентов с бэкендом (через gRPC-Web).

### Дополнительные материалы

- Статья ([https://habr.com/ru/companies/m2tech/articles/652305/](https://habr.com/ru/companies/m2tech/articles/652305/))
</details>
...
- [https://ru.wikipedia.org/wiki/Список*кодов*состояния_HTTP](https://ru.wikipedia.org/wiki/Список_кодов_состояния_HTTP)
...
<meta
  ...
  content="default-src 'self'; script-src 'self' https://trustedscripts.com;"
/>
```
...
<details>
<summary><b>Что такое PWA?</b></summary>

## PWA (Progressive Web App)

Веб-приложение, которое использует набор современных браузерных API, чтобы вести себя как нативное приложение — устанавливаться на устройство, работать оффлайн, слать push-уведомления — при этом оставаясь обычным сайтом на HTML/CSS/JS.

## Ключевые технологии

### Service Worker

Скрипт, который браузер запускает в отдельном потоке, независимо от страницы. Перехватывает сетевые запросы, кэширует ресурсы, работает даже когда вкладка закрыта. Это основа для оффлайн-режима и push-уведомлений.
...
JSON-файл с метаданными: имя приложения, иконки, цвет темы, `display: standalone` (убирает браузерный UI при запуске с домашнего экрана).
...
**HTTPS** — обязателен, service worker не регистрируется по http (кроме localhost).

## Что это даёт

- **Установка** — кнопка "Добавить на главный экран", иконка как у нативного приложения.
- **Оффлайн-работа** — через кэширование в service worker (Cache API).
- **Push-уведомления** — через Push API + Service Worker.
- **Быстрая загрузка** — ресурсы отдаются из кэша, а не из сети.

## Ограничения

- **iOS/Safari** исторически хуже поддерживает PWA (push-уведомления появились только с iOS 16.4, и то с оговорками), в отличие от Android, где интеграция почти нативная.
- Доступ к части нативных API (Bluetooth, точная геолокация в фоне, биометрия) либо ограничен, либо отсутствует.
- В App Store/Google Play PWA напрямую не публикуется (нужны обёртки типа Capacitor/Trusted Web Activity).

## Когда использовать

PWA имеет смысл, если нужен веб-сайт + приложение без раздельной разработки под iOS/Android/Web (один кодбейс), а требования не завязаны на глубокую нативную интеграцию. Если критичны нативные API или производительность на уровне игр — лучше React Native/Flutter или чистый native.

## Дополнительные материалы

- [Доклад про PWA в нашем сообществе](https://t.me/c/2188056950/10526/13494)
</details>

---

<details>
<summary><b>Какие существуют метрики WebVitals и на что они влияют?</b></summary>

## Web Vitals

**Web Vitals** — это набор метрик от Google, которые измеряют качество пользовательского опыта на сайте: скорость загрузки, отзывчивость на действия и визуальную стабильность. Из них выделяют **Core Web Vitals** — три ключевые метрики, которые Google учитывает при ранжировании сайтов в поисковой выдаче.

## Core Web Vitals (основные метрики)

### LCP (Largest Contentful Paint)

**Что измеряет:** время отрисовки самого большого видимого элемента (обычно это главное изображение, баннер или крупный блок текста). Фактически показывает, как быстро пользователь видит основной контент страницы.

**Хорошее значение:** ≤ 2.5 сек.

**На что влияет:** воспринимаемая скорость загрузки. Если LCP большой — пользователю кажется, что сайт «тормозит».

**Как улучшить:** оптимизация изображений, приоритетная загрузка ключевых ресурсов (preload), ускорение ответа сервера, CDN.

### INP (Interaction to Next Paint)

**Что измеряет:** отзывчивость интерфейса — задержку между действием пользователя (клик, тап, нажатие клавиши) и визуальным откликом. С марта 2024 года заменил метрику **FID (First Input Delay)**.

**Хорошее значение:** ≤ 200 мс.

**На что влияет:** ощущение «живости» интерфейса. Долгий INP означает, что кнопки и элементы реагируют с заметной задержкой (часто из-за тяжёлого JS, блокирующего основной поток).

**Как улучшить:** дробление длинных задач, дебаунс/тротлинг обработчиков, вынос тяжёлых вычислений в Web Workers, уменьшение объёма JS.

### CLS (Cumulative Layout Shift)

**Что измеряет:** визуальную стабильность — насколько сильно элементы «скачут» и смещаются во время загрузки страницы.

**Хорошее значение:** ≤ 0.1.

**На что влияет:** удобство. Классический пример плохого CLS — когда пользователь хочет нажать кнопку, но в этот момент подгружается баннер и сдвигает контент, из-за чего клик попадает не туда.

**Как улучшить:** задавать явные размеры (`width`/`height`) для изображений и медиа, резервировать место под динамический контент и рекламу, избегать вставки элементов над уже отрисованным контентом.
...
- **TBT (Total Blocking Time)** — суммарное время блокировки основного потока (лабораторный аналог INP).

## На что влияют Web Vitals в целом

1. **SEO** — Core Web Vitals являются фактором ранжирования Google, влияют на позицию сайта в поиске.
2. **Конверсия и удержание** — быстрые и стабильные сайты имеют меньший показатель отказов и большую конверсию.
3. **UX** — напрямую отражают то, насколько комфортно пользователю взаимодействовать с сайтом.

## Дополнительные материалы

- [https://web.dev/articles/vitals](https://web.dev/articles/vitals)

- [https://developer.mozilla.org/en-US/docs/Web/Performance/Guides/Web_performance_basics](https://developer.mozilla.org/en-US/docs/Web/Performance/Guides/Web_performance_basics)
