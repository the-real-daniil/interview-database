...
<details>
<summary><b>Что такое React Fiber?</b></summary>

`React Fiber` — это внутренний движок и алгоритм согласования (reconciliation) в библиотеке React, представленный в 16-й версии. Его главная задача — управлять процессом обновления интерфейса, разбивая тяжелые операции на мелкие части, чтобы интерфейс оставался отзывчивым при любых нагрузках.

### Как это работает?

В старых версиях React дерево компонентов обрабатывалось синхронно в один непрерывный проход. Если дерево было огромным, браузер «зависал» (пропускал кадры анимации), так как главный поток был занят.

React Fiber решает эту проблему следующими методами:

- Инкрементный рендеринг: Задача разбивается на небольшие фрагменты. React выполняет часть работы, а затем проверяет, не появились ли более приоритетные задачи (например, клик пользователя или ввод текста).
- Приоритеты обновлений: Система может ставить задачи в очередь: например, анимация обновляется мгновенно, а загрузка данных из сети или фоновый рендеринг откладываются до момента, когда браузер свободен.
- Приостановка и отмена: Работа над компонентом может быть приостановлена, отменена или начата заново в зависимости от того, как изменилось состояние приложения.

---

### Дополнительные материалы

- Статья ([https://habr.com/ru/articles/444276/](https://habr.com/ru/articles/444276/))

- YouTube ([https://www.youtube.com/watch?v=iJXBEjlSjg4&t=333s](https://www.youtube.com/watch?v=iJXBEjlSjg4&t=333s))

</details>

<details>
<summary><b>Что такое Concurrent в React?</b></summary>

`React Concurrent` (или конкурентный рендеринг) — это фундаментальный механизм React, который позволяет прерывать, приостанавливать или отменять процесс отрисовки компонентов. Он делает интерфейсы «умнее» и отзывчивее, позволяя браузеру выполнять срочные задачи (например, ввод текста) без ожидания завершения тяжелых фоновых вычислений.

### Как это работает простыми словами

- Неблокирующий рендеринг: Традиционный рендеринг работает синхронно — если компонент тяжелый, страница «зависает» (блокируется).
- Конкурентность позволяет React рендерить компоненты «в фоне», периодически проверяя, не появились ли более важные действия от пользователя.
- Приоритеты: React делит обновления на срочные (например, нажатие на кнопку или ввод в инпут) и второстепенные (загрузка списка постов). Срочные обновления могут «обогнать» фоновые вычисления.

---

### Дополнительные материалы

- Дока ([https://legacy.reactjs.org/blog/2022/03/29/react-v18.html](https://legacy.reactjs.org/blog/2022/03/29/react-v18.html))

- Статья ([https://habr.com/ru/companies/yandex/articles/514016/](https://habr.com/ru/companies/yandex/articles/514016/))

- YouTube:
  - [Что такое Concurrent в React ??? Глава 1](https://www.youtube.com/watch?v=J3Nwk9QUIvw&t=15s).
  - [Почему нам пришлось переписывать проекты. Глава 2](https://www.youtube.com/watch?v=GWuVdbtK54Q).
  - [Чем закончилась история с Concurrent Mode? Глава 3](https://www.youtube.com/watch?v=y7VSSLDLCMc)
...
Значение input'а хранится в state React, и React — единственный источник правды. Изменения идут через `onChange` → `setState` → ре-рендер с новым `value`.
...
- Значение всегда синхронизировано с state — можно валидировать, форматировать, блокировать ввод в реальном времени (например, разрешить только цифры).
- Каждое нажатие клавиши — ре-рендер компонента (и всех детей, если не оптимизировано через `memo`/раздельные компоненты полей).
- Легко реализовать зависимые поля (например, поле "город" зависит от выбранной "страны").

## Неконтролируемые компоненты

Значение хранится в DOM самим браузером, а не в React state. Доступ к значению — через `ref`, обычно в момент сабмита.
...
- `defaultValue` вместо `value` — задаёт начальное значение, но дальше DOM управляет им сам.
- Нет ре-рендеров при вводе — производительность лучше на больших/сложных формах.
- Единственный вариант для `<input type="file">` — его `value` нельзя контролировать программно по соображениям безопасности.

## Когда что использовать

**Контролируемые — по умолчанию, если:**

- Нужна валидация в реальном времени или форматирование ввода (маски телефона, лимит символов).
- Есть зависимые поля или условная логика (показать/скрыть поле в зависимости от значения другого).
- Нужно программно менять значение (сброс формы, автозаполнение).
- Небольшие/средние формы, где ре-рендеры не критичны.

**Неконтролируемые — когда:**

- Простая форма без сложной логики — просто собрать данные при сабмите.
- `<input type="file">` — обязателен.
- Критична производительность (сотни полей на странице, каждый ре-рендер дорог).
- Интеграция со сторонней non-React библиотекой, которая сама управляет DOM.

**На практике:** для реальных форм чаще берут библиотеки типа `react-hook-form`, которые используют неконтролируемый подход под капотом (через `ref`), но дают API как у контролируемых — валидацию, обработку ошибок и т.д., без ре-рендера на каждое нажатие. Это снимает дилемму «контролируемое vs неконтролируемое» для большинства практических случаев и особенно оправдано на формах с 10+ полями, где чистые контролируемые компоненты начинают заметно тормозить.
...
<details>
<summary><b>Что нового привнесла 19 версия React?</b></summary>

Наиболее значимые изменения 19 версии:

- Компонент `Activity`
  Это новый встроенный компонент (эволюция экспериментального Offscreen API). Он позволяет скрывать части дерева компонентов (аналогично display: none), не размонтируя их.
  Зачем нужно: Сохраняет внутреннее состояние (state), фокус и DOM-узлы скрытых элементов. Идеально подходит для реализации вкладок (tabs), модальных окон, которые нужно быстро открывать, или фоновой предварительной отрисовки.
- `ref` как обычный проп (Prop)
  Теперь ref можно передавать в функциональные компоненты напрямую, как любой другой проп.
  Зачем нужно: Полностью отпадает необходимость в обертке React.forwardRef. Это значительно упрощает код, делает его чище и удобнее для типизации в TypeScript. Также для ref была добавлена clean-up функция.
- `React Compiler` - Компилятор (ранее известный как React Forget) стал главной рекомендацией для оптимизации. Он автоматически анализирует ваш код и добавляет мемоизацию на этапе компиляции.
  Зачем нужно: Избавляет разработчиков от ручной и часто ошибочной расстановки useMemo и useCallback. React сам понимает, какие значения изменились, и предотвращает лишние ре-рендеры, сохраняя при этом семантику JavaScript. Устанавливается отдельным плагинов (НЕ из коробки).

---

### Дополнительные материалы

- Дока ([https://react.dev/blog/2024/12/05/react-19](https://react.dev/blog/2024/12/05/react-19))

- Статья ([https://habr.com/ru/articles/865416/](https://habr.com/ru/articles/865416/))

- Дока по компоненту Activity ([https://react.dev/reference/react/Activity](https://react.dev/reference/react/Activity))

- Статья по Activity ([https://habr.com/ru/companies/timeweb/articles/908488/](https://habr.com/ru/companies/timeweb/articles/908488/))

- YouTube ([https://www.youtube.com/watch?v=4pZpbrQ_HcY](https://www.youtube.com/watch?v=4pZpbrQ_HcY))
...
![React Hook Flow Diagram](images/react-hook-flow.png)
...
<details>
<summary><b>God Component</b></summary>

Антипаттерн «God Component» в React — это ситуация, когда один компонент берет на себя слишком много ответственности. Он превращается в гигантский, монолитный блок кода, который управляет огромным количеством состояния, сложной бизнес-логикой, побочными эффектами (side effects) и отрисовкой множества разных частей интерфейса.
Этот термин происходит от общего антипаттерна в программировании «God Object» (Божественный объект), который нарушает принцип единственной ответственности (Single Responsibility Principle, SRP).

### Основные признаки God Component:

- Огромный размер: Компонент занимает сотни или даже тысячи строк кода.
- Раздутое состояние: Десятки переменных в useState, огромные объекты в useReducer или бесконечные цепочки useEffect.
- Глубокая вложенность и условия: Множество вложенных тернарных операторов (condition ? A : B) и ранних возвратов (if (!data) return ...), из-за которых сложно понять, что именно рендерится.
- Смешение ответственности: Один компонент одновременно запрашивает данные с API, обрабатывает формы, управляет глобальным состоянием, анимациями и отрисовывает дюжину разных UI-элементов.
- Сложность тестирования: Чтобы протестировать маленькую часть логики, приходится мокать (mock) половину зависимостей компонента.
  Почему это плохо?
- Нарушение SRP: Компонент делает слишком много всего, поэтому любая правка может сломать непредсказуемую часть функционала.
- Проблемы с производительностью: При изменении любого кусочка состояния (даже не связанного с видимой частью экрана) весь огромный компонент перерисовывается (re-render), что вызывает лаги.
- Сложность поддержки и онбординга: Новому разработчику (или вам самим через полгода) крайне сложно разобраться в этом «спагетти-коде».
  Конфликты при слиянии (Merge Conflicts): Если над одним файлом работают несколько человек, Git-конфликты становятся неизбежными.

</details>

<details>
<summary><b>useState и useEffect для синхронных данных</b></summary>

Частая ошибка: использовать хуки useState и useEffect для вычисления производных, синхронных значений, когда все необходимые данные уже доступны при рендере.
...
- Статья ([https://react.dev/learn/you-might-not-need-an-effect](https://react.dev/learn/you-might-not-need-an-effect))

</details>

<details>
<summary><b>Использование useState вместо useRef, если не нужен повторный рендеринг компонента</b></summary>

### Почему это плохо

- Потери производительности: Повторные рендеры без необходимости нагружают React и могут замедлить крупные приложения;

- Неочевидная логика: Использование useState ради флага, который не влияет на отображение, запутывает логику компонента и усложняет понимание кода.
</details>

<details>
<summary><b>Топ-n антипаттернов</b></summary>

Каждый разработчик выделяет свой топ антипаттернов и ранжирует их по-своему. Ниже приведены материалы с обзорами на популярные ошибки, избегая которых Вы сможете сэконоить кучу нервов себе и своим будущим коллегам :D

---

### Дополнительные материалы

- Статья ([https://habr.com/ru/articles/937656/](https://habr.com/ru/articles/937656/))

- YouTube ([https://www.youtube.com/watch?v=b2o-iJzAfTc](https://www.youtube.com/watch?v=b2o-iJzAfTc))
...
<details>
<summary><b>Что такое батчинг?</b></summary>
Батчинг — это механизм React, который объединяет несколько последовательных вызовов функции обновления состояния (например, setState) в один общий цикл рендеринга. Это оптимизирует производительность приложения, избавляя от лишних и ресурсоемких перерисовок экрана. Появился в 18 версии библиотеки.

---

### Дополнительные материалы

- Дока ([https://react.dev/learn/queueing-a-series-of-state-updates](https://react.dev/learn/queueing-a-series-of-state-updates))

- Статья ([https://habr.com/ru/articles/577168](https://habr.com/ru/articles/577168))

- YouTube ([https://www.youtube.com/watch?v=VfQ-qSjIalU](https://www.youtube.com/watch?v=VfQ-qSjIalU))
...
<details>
<summary><b>Для чего используется компонент ErrorBoundary в React?</b></summary>

`ErrorBoundary` (граница ошибок) — это специальный компонент в React, который отлавливает JavaScript-ошибки, возникающие в дочерних компонентах, предотвращает крах всего приложения и отображает запасной пользовательский интерфейс (например, сообщение о сбое).

### Зачем нужен

- Изоляция сбоев: Ошибка в одном незначительном блоке интерфейса (например, поломкажет виджета отзывов) не приводит к падению всей страницы.
- Запасной UI: Вместо пустого белого экрана или бесконечной загрузки пользователь видит понятное сообщение.
- Логирование: Позволяет автоматически отправлять данные об ошибках в сервисы мониторинга (например, Sentry) для отладки.

### Что НЕ перехватывает

Error boundary не обрабатывают ошибки в следующих случаях:

- Обработчики событий (например, onClick) — для них нужно использовать стандартный try/catch.
- Асинхронный код (например, колбэки setTimeout или промисы).
- Серверный рендеринг (SSR).
- Ошибки, происходящие в самом компоненте Error Boundary (а не в его дочерних элементах).

---

### Дополнительные материалы

- Дока ([https://react.dev/reference/eslint-plugin-react-hooks/lints/error-boundaries](https://react.dev/reference/eslint-plugin-react-hooks/lints/error-boundaries))

- Статья ([https://habr.com/ru/articles/661413/](https://habr.com/ru/articles/661413/))

- YouTube ([https://www.youtube.com/watch?v=gyqAW0--0Tc&t=16s](https://www.youtube.com/watch?v=gyqAW0--0Tc&t=16s))
...
Все четыре стратегии описывают, **когда** формируется HTML. Отдельная от них тема — React Server Components (в App Router они используются по умолчанию): см. вопрос «Что такое React Server Components и чем они отличаются от SSR?» ниже.
...
| | SSR | RSC |
|---|---|---|
| Где выполняется | На сервере, затем повторно на клиенте при гидрации | Только на сервере, один раз |
| Код компонента в бандле | Да | Нет |
| Что уезжает на клиент | HTML + JS компонента | RSC payload (описание дерева) |
| Состояние и обработчики | Есть после гидрации | Нет |
| Когда выполняется | На каждый запрос страницы | На запрос страницы или на навигацию, ответ можно кэшировать |

RSC — не замена SSR, а дополнение: обычно они работают вместе. Первый ответ сервера всё так же собирается в HTML (это делает SSR), а RSC определяет, чей код при этом вообще нужен браузеру.

### Что это даёт

- **Прямой доступ к данным.** Внутри серверного компонента можно ходить в БД, читать файлы и использовать секреты — это никогда не окажется в браузере.

- **`async`-компоненты.** Серверный компонент может быть асинхронным и делать `await` прямо в теле, без `useEffect` и без стейта загрузки.

- **Меньше JS.** Тяжёлые зависимости (парсер markdown, библиотека дат, SDK) остаются на сервере и не увеличивают бандл.

```tsx
// app/post/[id]/page.tsx — серверный компонент (по умолчанию в App Router)
...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <article>
      <h1>{post.title}</h1>
      <p>{post.body}</p>
      {/* Интерактивная часть вынесена в клиентский компонент */}
...
- Нет состояния и хуков состояния/эффектов: `useState`, `useReducer`, `useEffect`, `useContext` недоступны.
- Нет обработчиков событий: `onClick`, `onChange` и т.п. передать нельзя.
- Нет доступа к браузерным API: `window`, `localStorage`, `document`.
- Нельзя использовать классовые компоненты.

Всё это — не баг, а следствие того, что компонент отработал на сервере и больше не «живёт» на клиенте.

### Граница `'use client'`

Директива `'use client'` в начале файла помечает **точку входа в клиентскую часть**: сам модуль и всё, что он импортирует, попадают в бандл и гидрируются.
...
Два следствия, о которых чаще всего спрашивают:

- **Пропсы должны быть сериализуемыми.** Из серверного компонента в клиентский можно передать строки, числа, массивы, объекты, `Date`, промисы — но не функции и не классы: передать колбэк вниз через границу нельзя.

- **Серверный компонент внутри клиентского возможен только через `children`/пропсы.** Клиентский компонент не может `import`-ировать серверный (тогда его код пришлось бы отправить в браузер), но может отрендерить уже готовое серверное дерево, полученное пропсом:

```tsx
// Работает: <ServerWidget /> собран на сервере и передан как children
<ClientLayout>
  <ServerWidget />
</ClientLayout>
```

### Связь с Server Actions и Suspense

- **Server Actions (`'use server'`)** — обратное направление: функция, которая объявлена на сервере, но может быть вызвана из клиентского компонента (например, из формы). Так интерактивность возвращается туда, где нет клиентского кода для мутаций.

- **`Suspense`** — способ стримить серверные компоненты по мере готовности: медленная часть страницы отдаётся позже, остальное показывается сразу. Про сам компонент есть отдельный вопрос — «Для чего используется компонент Suspense в React?» выше.

### Типичные уточняющие вопросы

- **RSC заменяют SSR?** Нет, дополняют: RSC решают, какой код нужен клиенту, SSR — как отдать первый HTML. В Next.js они работают вместе.

- **Нужен ли фреймворк?** Практически да: RSC требуют бандлера и роутера, умеющих строить и передавать RSC payload (Next.js App Router, Parcel/RSC-сборки, Waku). Голым `react-dom` это не включается.

- **Как это соотносится с SSG/ISR?** Ортогонально: серверный компонент может выполниться на этапе сборки (SSG), по запросу (SSR) или с кэшированием и ревалидацией (ISR) — стратегия кэширования от RSC не зависит.

- **Почему «функция не сериализуется»?** Значит, из серверного компонента в клиентский попытались передать колбэк. Решение — либо поднять состояние в клиентский компонент, либо использовать Server Action.

---

### Дополнительные материалы

- Дока ([https://react.dev/reference/rsc/server-components](https://react.dev/reference/rsc/server-components))

- Next.js ([https://nextjs.org/docs/app/getting-started/server-and-client-components](https://nextjs.org/docs/app/getting-started/server-and-client-components))

- Статья ([https://habr.com/ru/companies/otus/articles/778924/](https://habr.com/ru/companies/otus/articles/778924/))
