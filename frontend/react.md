...
- Конкурентность позволяет React рендерить компоненты «в фоне», периодически проверяя, не появились ли более важные действия от пользователя.Приоритеты: React делит обновления на срочные (например, нажатие на кнопку или ввод в инпут) и второстепенные (загрузка списка постов). Срочные обновления могут «обогнать» фоновые вычисления.
