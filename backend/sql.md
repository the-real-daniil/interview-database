...
| `SELECT *` | читаются TOAST-поля и лишние колонки, растёт объём I/O и трафик | перечислить только нужные колонки, тогда возможен `Index Only Scan` |
| `ORDER BY total DESC` | сортировка суточного среза (сотни тысяч строк) в памяти/на диске | включить `total` в индекс — сортировка исчезает из плана |
| Нет индекса под фильтр | планировщику нечего выбрать | составной индекс `(status, created_at, total DESC)` |

### Порядок колонок в составном индексе

Правило: сначала колонки под **равенство**, потом под **диапазон**, потом под **сортировку**. `status` сравнивается через `IN` (набор равенств), `created_at` — диапазон, `total` — сортировка. Если поставить `created_at` первым, отсортированность по `total` внутри индекса потеряется и `Sort` в плане останется.
...
- **Частичный индекс**, если отчёт всегда по одним и тем же статусам: `CREATE INDEX ... ON orders (created_at, total DESC) WHERE status IN ('paid','shipped')` — индекс меньше и точнее.
