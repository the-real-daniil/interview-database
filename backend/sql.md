...
Дополнительно полезно порешать:

- [SQL Academy](https://sql-academy.org/ru/trainer) — задачи с проверкой прямо в браузере на русском, удобно добивать конкретные темы (джойны, подзапросы, агрегации) точечно;
- [pgexercises.com](https://pgexercises.com/) — задачи на PostgreSQL с подробными разборами решений.

И обязательно почитать вывод `EXPLAIN ANALYZE` на своих реальных таблицах — на собеседовании чаще спрашивают не синтаксис, а умение объяснить план запроса.
...
При подходящем индексе `(user_id, status, total DESC)` `LATERAL` может быть существенно эффективнее, если внешний запрос возвращает относительно небольшое число пользователей: для каждого пользователя можно быстро получить первые 3 подходящих заказа, прочитав ровно 3 строки индекса, а не всю партицию.
...
| `NOT IN (SELECT user_id ...)` | **опасен**: если в подзапросе есть хоть один `NULL`, результат всегда пустой | современный планировщик PostgreSQL умеет хорошо оптимизировать и эту форму — но риск с `NULL` остаётся главной причиной избегать `NOT IN` с подзапросом |
