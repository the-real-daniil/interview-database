...
- **Почему для аналитики колоночное хранение быстрее?** Читаются только нужные колонки, а однотипные значения в колонке отлично сжимаются — на порядок меньше I/O.
...
- при `SELECT *` TOAST читается всегда, даже если колонка не нужна — одна из причин перечислять колонки явно.
...
Счётчик транзакций 32-битный, поэтому старые версии нужно периодически «замораживать» (`FREEZE`). Если autovacuum не справляется, PostgreSQL сначала предупреждает, а затем **останавливает запись** в базу, чтобы не потерять данные. Мониторинг `age(datfrozenxid)` — обязательная метрика на проде.
...
|---|---|---|
| `Seq Scan` | таблица маленькая или нужна большая доля строк | на большой таблице с селективным фильтром — нет индекса или предикат не sargable |
...
- **Что такое failover и split-brain?** Переключение на реплику при падении primary; split-brain — ситуация, когда два узла считают себя primary. Защита — кворум и fencing (Patroni, etcd).
...
- **Как реализовать оптимистическую блокировку в ORM?** Встроенные механизмы: `@VersionColumn` в TypeORM, `@version` в Prisma-подобных решениях делают проверку версии в `WHERE` автоматически.
...
Между публикацией и отметкой `published_at` процесс может упасть, поэтому событие иногда уйдёт дважды. **Потребители обязаны быть идемпотентными** (дедупликация по `event_id`). Exactly-once на практике достигается именно так: at-least-once доставка + идемпотентная обработка.
...
- **Среда исполнения.** Serverless и edge требуют лёгкого клиента и внешнего пула соединений (Drizzle и Prisma с data proxy/PgBouncer).
