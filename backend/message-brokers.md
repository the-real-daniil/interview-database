...
В модели очереди сообщение доставляется **одному** консьюмеру. В RabbitMQ оно удаляется сразу после `ack`; в BullMQ джоб не удаляется, а переходит в состояние `completed`/`failed` и хранится согласно retention-настройкам (`removeOnComplete`/`removeOnFail`). В модели лога сообщение пишется в персистентный журнал, живёт там до истечения retention и может быть прочитано **любым числом** независимых подписчиков, каждый со своей позицией (Kafka).
...
Гарантий три: **at-most-once** (не больше одного раза, возможна потеря), **at-least-once** (не меньше одного раза, возможны дубликаты) и **exactly-once** (ровно один раз). Exactly-once **доставка** на уровне сети недостижима в принципе (потерянный ACK неотличим от потерянного сообщения, а значит ретрай неизбежен) — это фундаментальное ограничение, а не недоработка конкретного брокера. Но exactly-once **обработка** (эффект «как будто ровно один раз») достижима, если переложить идемпотентность на внешние системы: dedup-таблицу по ключу сообщения, транзакционный outbox, Kafka-транзакции. Поэтому в продакшене почти всегда берут at-least-once на уровне доставки и добавляют **идемпотентность** обработчика.
...
| Kafka `enable.idempotence=true` | дедупликация **ретраев продюсера** внутри партиции (PID — Producer ID, идентификатор продюсера, который Kafka выдаёт при включении этой настройки — + sequence number) |
...
В классическом RabbitMQ retry-механизм не является частью модели доставки — его обычно собирают из **dead-letter exchange (DLX)**, **TTL сообщения** и отдельной retry-очереди, либо через плагины (например `rabbitmq-delayed-message-exchange`). Схема такая: неуспешное сообщение отклоняется с `requeue=false`, попадает по DLX в очередь-задержку с TTL, а по истечении TTL само возвращается в основную очередь.
...
- Переживает падение меньшинства узлов при условии, что большинство (кворум по Raft) остаётся доступным: кластер из 3 узлов терпит потерю одного, из 5 — двух. Если из-за сетевого разделения большинство недостижимо, очередь перестаёт принимать записи — это осознанный компромисс в пользу консистентности. Число узлов берут нечётным, чтобы не было split-brain (ситуации, когда кластер разделяется на части, и в каждой части выбирается свой лидер, из-за чего появляются конфликтующие записи).
...
- Per-message TTL и приоритеты сообщений поддерживаются: TTL — наравне с queue-level TTL, а начиная с RabbitMQ 4.3 — строгий приоритет с 32 уровнями (0–31), без opt-in аргумента `x-max-priority` (он релевантен только для classic queues, quorum queues его игнорируют). При миграции со старых версий RabbitMQ (до 4.3) стоит проверять, что приоритеты действительно нужны.
...
- Партиция выбирается по хешу ключа: сообщения с одним `order_id` всегда попадают в одну партицию, значит их порядок сохранён. Без ключа выбор зависит от partitioner: современные продюсеры используют не строгий round-robin, а sticky partitioning — пока не наберётся батч, все сообщения идут в одну партицию, и только потом выбор переключается на следующую, — так партиции заполняются более крупными батчами.
...
| Судьба сообщения | переходит в completed/failed (retention настраивается) | удаляется после ack | живёт до конца retention |
...
| Throughput | десятки тысяч/мин | десятки тысяч/с | сотни тысяч–миллионы/с (при батчинге, достаточном числе партиций и тюнинге кластера) |
