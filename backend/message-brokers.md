...
- Переживает падение меньшинства узлов: кластер из 3 узлов терпит потерю одного, из 5 — двух. Число узлов берут нечётным, чтобы не было split-brain (ситуации, когда кластер разделяется на части, и в каждой части выбирается свой лидер, из-за чего появляются конфликтующие записи).
